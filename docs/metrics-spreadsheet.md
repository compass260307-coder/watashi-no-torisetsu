...
- `table=events`（既定）… events テーブルの行。列: `created_at, date_jst, event_name, session_id, invite_code, owner_token, metadata`
- `table=users` … users テーブルの行。列: `created_at, date_jst, id, type_id, campaign, generation, invite_code, source_user_id`
