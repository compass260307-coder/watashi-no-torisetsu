-- Alice (/hoshiyomi) の管理画面分析用イベント。
-- 会話本文やユーザー識別子は events に入れず、導線と成否だけ記録する。
...
create index if not exists idx_hoshiyomi_credit_reservations_created_status_user
  on public.hoshiyomi_credit_reservations (created_at, status, user_id);

create index if not exists idx_hoshiyomi_conversations_created_user
  on public.hoshiyomi_conversations (created_at, user_id);
