// 第二部「友達から見たあなた」本文のサーバ解決 (三層モデル Step2)。
//
// 構成 (2026-07-12 確定):
//   1. 友達から見たあなたの武器      … 無料 (未解放でも公開。バイラルの燃料)
//   2. 友達から嫌われやすい性格      … 🔒 友達3人 or 完全版
//   3. 友達から好かれやすい性格      … 無料 (未解放でも公開)
//   4. 関係別の見られ方 (友達/恋人/家族/上司) … 🔒 友達3人 or 完全版
//
...
const DISLIKE_TAIL = [
  ...
  "これは短所というより、あなたの長所が強く出すぎた瞬間。自覚しておくだけで武器に変わる。",
...
const LIKABLE_PROSE: Record<BigFiveDimension, Record<"H" | "L", string>> = {
  E: {
    H: "あなたの周りは、いつも少しにぎやか。あなたが来るだけで空気が軽くなって、「とりあえず呼ぼう」の一番手に挙がっている。本人が思っているより、その明るさに救われている友達はずっと多い。",
    L: "あなたの魅力は、静かな安心感。騒がないぶん、話をちゃんと受け止めてくれる人だと思われていて、二人きりのときのあなたが一番好かれている。「あの子には話せる」——そう思っている友達が、たぶんいる。",
  },
  A: {
    H: "それから、頼まれる前に動ける自然な気配り。あなたにとっては当たり前すぎて気づいていないけれど、その小さな優しさに周りは何度も救われている。",
    L: "それから、裏表のなさ。思ったことをそのまま言うから、あなたの褒め言葉には嘘がない。だからこそ、あなたに認められると特別に嬉しい——そんな存在になっている。",
  },
  C: {
  ...
  O: {
    H: "話していて飽きない、というのもあなたの得なところ。「それ何?」から会話がどんどん転がっていくから、一緒にいる時間が長いほど好かれていく。",
    L: "流行に流されない安定感も、あなたの得なところ。いつ会っても同じテンションでいてくれる心地よさは、長く付き合うほど効いてくる。",
  },
  N: {
    H: "そして、誰かが沈んでいる時に最初に気づくのは、たぶんあなた。何気ないその一言に救われた友達が、きっといる。",
    L: "そして、何が起きても慌てない余裕。そばにいるだけでなんだか安心する、と思われている。",
  },
};

const LIKABLE_CLOSING =
  "本人は無自覚かもしれないけれど、この組み合わせはかなり得な性格です。無理に盛らなくていい。いつものあなたのままで、ちゃんと好かれています。";
...
const RELATION_FRIEND: Record<Quad, string> = {
  // E × A
  HH: "グループの中心にいる、ノリが良くて優しいやつ。遊びの誘いはまずあなたに声がかかるし、あなたが来る日は集まりの出席率が上がる。",
  ...
  LL: "群れない、自分の世界を持ってるやつ。付き合う相手は狭いけど、あなたが心を許した友達は、それをちょっと誇りに思ってる。",
};

const RELATION_LOVER: Record<Quad, string> = {
  // A × N
  HH: "相手の気持ちを先回りして動く、尽くすタイプ。その優しさは伝わってる。ただ我慢も一緒に溜めがちだから、恋人はあなたの本音をもっと聞きたいと思ってるかも。",
...
const SCENE_FRIEND: Record<Quad, string> = {
  // E × A
  HH: "頼まれごとを断れず、幹事も相談役も全部引き受けてパンクしがち。あなたが疲れている日ほど、周りは気づかず甘えてくる。月に一度は「今日は無理」と言う練習を。",
  ...
  LL: "連絡を放置しているうちに、悪気なく疎遠になりがち。あなたは「変わらず友達」のつもりでも、相手には「離れた」と映ることがある。年に数回のひと言で、関係は保てる。",
};
...
const SCENE_LOVER: Record<Quad, string> = {
  ...
  LL: "安定しているぶん、愛情表現が省エネになりがち。あなたの「言わなくても分かるでしょ」は、相手には「もう好きじゃないのかも」に変換される。定期的な「好き」はメンテナンス。",
};

const SCENE_CAREER: Record<Quad, string> = {
  // C × N
  HH: "完璧主義と気にしすぎの組み合わせで、自分を追い込みやすい。100点の準備より、60点で出して直す方が評価される場面は多い。「これで十分」のラインを先に決めておこう。",
  HL: "仕事は回せるぶん、限界を超えても気づかず走り続けてしまう。それと、ルーズな人への無言の圧にも注意。あなたの「普通」は、けっこう高い基準。",
...
export function resolvePartTwo(
  ...
  const gapTeaser = `あなたが自分でいちばん強いと思っている「${AXIS_LABEL[topDim]}」。友達の目には、違う濃さで映っているかも。`;
