...
export function buildMinnaProse(
  ...
  if (diff >= 8) {
    ...
    paras.push(
      `${friendWord}の目には、自分が思うよりずっと「${gap.label}のある人」として映っているみたい。その${gap.label}を、あなたが思う以上に${who}は頼りにしているよ。自分では当たり前にやっていることが、周りにはしっかり届いているんだ。`,
    );
  } else if (diff <= -8) {
    ...
    paras.push(
      `「${gap.label}」の見え方は、自分と${who}でほとんど同じ。自己イメージと周りの印象がきれいに重なっているのは、あなたが素のままで人と関われている証拠だよ。`,
    );
  }
  ...
  paras.push(
    agreement >= 70
      ? `${who}との見方の一致は${agreement}%。自分らしさが、そのまま周りに伝わっているみたい。今のあなたのままで、まわりはちゃんと受け取ってくれているよ。`
...
> = {
  開放性: {
    ...
    higher:
      "{name}と一緒のとき、あなたは自分で思うよりフットワークが軽くなるみたい。「ちょっと気になる」とつぶやいたお店に、気づけばふたりで並んでいる——そんな展開が多いはず。",
    lower:
      "本当は気になっているイベントやお店を、あなたはまだ{name}に言えていないのかも。「実はこれ行きたくて」と見せた瞬間、ふたりの週末は一気ににぎやかになるよ。",
  },
  誠実性: {
    match:
      "テスト前の計画も、旅行のしおりも、ふたりの「ちゃんとやる度」はほぼ同じ。集合時間の感覚まで似ているから、待ち合わせでイライラした記憶がほとんどないんじゃないかな。",
    higher:
      "課題や約束をきっちり守るあなたの姿を、{name}は横でちゃんと見てる。「この人に任せれば大丈夫」——グループワークでそう思われているのは、たぶんあなただよ。",
    lower:
      "あなたなりの段取りは、{name}には少し自由に見えているみたい。旅行や大事な予定のときだけ「ここは決めておこう」と先に握っておくと、ふたりの時間はもっとスムーズになるよ。",
  },
  外向性: {
    match:
      "オールではしゃぐ日も、家でだらだら動画を見る日も、ふたりのテンションは自然にそろう。「今日はどっちの気分?」と確認しなくていいのが、この関係のいちばん楽なところ。",
    higher:
      "{name}の前のあなたは、ふだん見せている顔よりずっとよく笑ってるみたい。疲れた日の帰り道、{name}と話しているうちに元気になってた——そんな経験、きっとあるはず。",
    lower:
      "{name}にはあなたが少し物静かに映っているみたい。カラオケでもドライブでも、本気ではしゃぐあなたを見せたら、{name}はもっと喜ぶよ。",
  },
  協調性: {
    match:
      "お店を決めるときも、割り勘のときも、どっちが我慢するでもなく自然に折り合える。「合わせすぎて疲れた」がないのは、気づかいの温度が同じだからだよ。",
    higher:
      "グラスが空いたら注ぐ、疲れてそうなら早めに切り上げる——あなたのさりげない気づかいを、{name}は全部ちゃんと受け取ってる。今度は遠慮せず、頼る側にも回ってみて。",
    lower:
      "あなたの気づかいは、{name}にはまだ半分しか届いていないかも。「荷物持つよ」「大丈夫?」を言葉にした日から、{name}の中のあなたはもっとあったかい人になるよ。",
  },
  神経症傾向: {
    match:
      "落ち込んだ夜に長文を送っても引かれない安心感がある。励ましの言葉がなくても、通話をつないでおくだけで回復できる。しんどい日の過ごし方まで似ているふたりだよ。",
    higher:
      "「平気だよ」と笑った日の帰り際に「ほんとに?」と聞いてくるのが{name}。あなたの強がりは、たぶんもうバレてる。先に弱音を吐いちゃうほうが、このふたりはうまくいくよ。",
    lower:
      "{name}の前では、あなたはいつも落ち着いて見えているみたい。それだけ安心できる相手ということ。でも無理してる日は正直に言って大丈夫、ちゃんと受け止めてくれるよ。",
  },
};
...
function estimatedSummaryParas(percent: number, viewer: string): string[] {
  if (percent >= 85) {
    return [
      `回答を見るかぎり、${viewer}はあなたのことをかなり正確に見てる。わかってもらえてる安心感があるから、素のままで居られる相性だよ。`,
      `「わかってくれてる」って、実は当たり前じゃない。${viewer}の回答は、あなたが言葉にしていない部分までちゃんと見てくれている証拠。この安心感は、ふたりの関係のいちばんの武器だよ。`,
    ];
  }
  if (percent >= 70) {
    return [
      `${viewer}の見ているあなたと、自分の思うあなたはだいたい重なってる。ときどき見え方のズレはあるけど、それが会話のネタになるくらいのいい距離感。`,
      `全部が一致していないのが、むしろちょうどいい。わかり合えている土台があるから、ズレは「へえ、そう見えてたんだ」って楽しめる。話すたびに発見がある関係だよ。`,
    ];
  }
  if (percent >= 55) {
    return [
      `${viewer}から見えているあなたには、自分の認識とちょっと違う部分があるみたい。そのズレこそ、お互いをもっと知る伸びしろだよ。`,
      `ギャップは悪いことじゃなくて、${viewer}があなたの知らない一面を見つけてくれてるということ。気になったところは本人に聞いてみると、思った以上に会話が盛り上がるよ。`,
    ];
  }
  return [
    `${viewer}とは見え方のギャップが大きめ。それは合わないというより、意外な自分を教えてくれる貴重な存在ということでもあるよ。`,
    `大きなギャップは、${viewer}の前で見せている顔と、自分で思う顔が違うということ。どっちも本物のあなた。両方知っている人が増えるほど、あなたはもっと自由になれるよ。`,
  ];
}
