...
//   - 🔒第二部 (友達3人 or 完全版): キャリア(career) / 成長(growth) / 相性(aisho) タブ本文。
...
> = {
  N: {
    H: "あなたの恋人が静かに受け止めているのは、あなたの「考えすぎる夜」。既読の速さや語尾の変化を深読みして不安になったあなたの、「ねえ、ほんとに大丈夫?」——その確認に何度も同じ答えを返しながら、「どう伝えたら安心してもらえるんだろう」と、相手も相手で悩んでいたりする。",
    L: "あなたの恋人が静かに飲み込んでいるのは、小さな不安を打ち明けたときの「気にしすぎだって」のひと言。あなたに悪気がないのは分かっているから責めないけど、「この人には深刻に聞こえないんだな」と思うたび、少しずつ相談することをやめていく。",
  },
  A: {
    H: "それから、あなたの「どこでもいいよ」。合わせてくれる優しさだと分かっていても、本音が見えないことは、相手にとって案外さみしい。「本当はどうしたいの?」を聞き出せないまま、あなたの顔色から答えを探す作業を、恋人は密かに続けている。",
    L: "それから、あなたの「正しすぎる一言」。的確なのは分かっているから言い返さないけど、欲しかったのは答えじゃなくて「わかるよ」だった——そんな夜を、恋人は言わずに流している。あなたの率直さを好きでいるからこそ、なおさら口にしない。",
  },
  E: {
    H: "そして、あなたのスピードとにぎやかさ。予定がどんどん埋まっていくあなたの隣で、「二人だけのゆっくりした時間」を言い出すタイミングを、相手はそっと窺っている。",
    L: "そして、誘うのがいつも自分側だという小さな偏り。あなたの気持ちを疑ってはいないけど、「たまには先に誘ってほしい」を、相手は言えずにいる。",
  },
  C: {
    H: "あなたの「ちゃんとしたい」に合わせて、ゆるく過ごしたい日を少し我慢していることもある。",
    L: "あなたの返信の波を「気持ちの波じゃない」と自分に言い聞かせている日も、たぶんある。",
  },
};

const LOVE_ENDURE_CLOSING =
  "ここまで読んで胸がチクッとしても、大丈夫。相手が黙っているのは、あなたが嫌いだからじゃなく、この関係を壊したくないから。我慢の場所を先に知って、こちらから「最近どう?」と聞けたなら——その我慢は、ぜんぶ信頼に変わる。";
...
> = {
  E: {
    H: "職場でのあなたは、自然と輪の真ん中に近いポジションにいる人。誰とでもすぐ打ち解けて、空気が重い会議でも口火を切れる。ただ、広く付き合えるぶん「誰にでもいい顔をする」と見られたり、浅い関係ばかり増えて、本当にしんどいときに頼れる相手がいない——そんな瞬間が、思い当たらない?",
    L: "職場でのあなたは、雑談の輪にガンガン入っていくタイプじゃない。でもそれは壁があるんじゃなくて、少人数とじっくり信頼を積むスタイルなだけ。ただ、静かにしているだけで「何を考えてるか分からない」と誤解されたり、気づいたら輪の外に置かれていた——そんな損のしかたをすることがある。",
  },
  A: {
    H: "人間関係で一番すり減るのは、頼まれごと。嫌われたくなくて断れず、気づけばあなたの机にだけ仕事が積み上がっていく。衝突を避けて飲み込んだ「それ、おかしくない?」は消えずに溜まって、ある日どっと疲れに変わる。優しさが、いちばんの消耗ポイントになりやすい。",
    L: "あなたは思ったことを率直に言えるタイプ。それ自体はチームの財産だけど、正しい指摘ほど「キツい一言」として届くことがある。相手が欲しかったのは正解じゃなくて、まず「わかるよ」の一言だった——そんなすれ違いが、評価に響く前に知っておきたいクセ。",
  },
  N: {
...
const CAREER_RELATIONS_CLOSING =
  "人間関係のクセは、直すものじゃなくて「置く場所」の問題。あなたの距離のとり方がそのまま活きるチームなら、無理にキャラを変えなくても、ちゃんと信頼される。合う距離感の職場を選ぶことが、いちばんの処世術なんです。";
...
export function resolveDeepDiveSections(
  ...
  function scoreNote(hint: (typeof DEEP_DIVE_CARDS)[number]["hint"]): string {
    if (hint === "top") {
      return `あなたの中で最も高いのは${AXIS_LABEL[topDim]}（${pct[topDim]}%）。`;
    }
    if (hint === "bottom") {
    ...
    if (hint === "aisho") {
      return "32タイプ全員と総当たりで計算した、あなたの相性ランキング。";
    }
    return `あなたの${AXIS_LABEL[hint]}は${pct[hint]}%。`;
  }
