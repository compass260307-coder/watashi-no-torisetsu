...
function LineRegisterContent() {
  ...
  const performRegister = async (
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStatus("success");
        if (data.welcomeError) setWelcomeError(data.welcomeError);
        track("line_register_completed", {
          ownerToken: ownerToken ?? undefined,
          metadata: {
            lineUserId: profile.userId,
            welcomed: !!data.welcomed,
            welcomeError: data.welcomeError,
          },
        });
      } else {
        const data = await res.json().catch(() => ({}));
      ...
      console.error("line-register POST failed:", err);
      setStatus("error");
  ...
  const checkFriendshipAndProceed = async (
    ...
    try {
      ...
      // friendship チェック失敗時は念のため friend-add 案内へフォールバック
      setStatus("needs-friend-add");
    }
  };
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        await checkFriendshipAndProceed(liff, profile);
      } catch (err) {
  ...
  return (
    <div className="flex flex-col flex-1 items-center justify-center px-5 py-10">
      ...
      {(status === "init" || status === "checking-friendship") && (
        <>
      ...
      {status === "needs-friend-add" && (
        <>
          ...
          <a
            ...
            className="inline-flex items-center justify-center gap-2 w-full max-w-xs rounded-full bg-[#06C755] px-8 py-4 text-base font-bold text-white shadow-md shadow-[#06C755]/25 transition-all hover:bg-[#05b34a] active:scale-[0.98] mb-3"
          >
            <svg
              viewBox="0 0 24 24"
              className="w-5 h-5"
          ...
          <button
            ...
            className="rounded-full bg-primary-gradient px-8 py-3 text-sm font-bold text-white shadow-md disabled:opacity-50"
          >
            {checkingAgain ? "確認中..." : "追加完了の確認"}
          </button>
        </>
      )}
      ...
      {status === "success" && (
        <>
          ...
          {welcomeError ? (
            <p className="text-sm text-muted text-center mb-6 leading-relaxed">
              welcome メッセージの送信に失敗した可能性があります。
              <br />
              LINEトークを確認し、届いていない場合は
              <br />
              公式アカウントが友だち追加されているか
              <br />
              ご確認ください。
