...
export default async function EnglishReportPrintPage({ params }: Props) {
  ...
  return (
    <main className="report">
      <section className="page cover"><p className="eyebrow">ALICE TEST</p><h1>{type.name}</h1><p className="subtitle">{type.essence} · {type.animal}</p><p className="lead">{type.oneLiner}</p><div className="name">Prepared for {data.display_name?.trim() || "you"}</div><p className="edition">COMPLETE EDITION</p></section>
