// PR1: ¥99 買い切り「フルアクセス(全解放)」の Stripe Checkout Session 作成。
//
...
// Price は STRIPE_PRICE_FULL_ACCESS (¥99/one-time)。金額はサーバ側の Price 固定で、
...
const SALE_JPY = 99;
const LIST_JPY = 1299;
const DISCOUNT_JPY = LIST_JPY - SALE_JPY; // 1200
...
export async function POST(request: NextRequest) {
  ...
  // ===== 「買いたくなる」表示: 商品説明/画像 + 値引き表示 (¥1,299 → ¥99) =====
  // 安全設計: 値引き経路 (price_data LIST + クーポン) は
  //   ① Price ID の実額が SALE (¥99) と一致し ② クーポンが amount_off=DISCOUNT で解決
  ...
  // 値引き経路: price_data で LIST を出し、クーポンで DISCOUNT 引いて実額 SALE (=¥99) を課金。
