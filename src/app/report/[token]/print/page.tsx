...
import { buildIdDetailedReport } from "@/i18n/id/detailed-report";
    ...
    title: {
      absolute:
        sp.locale === "ko"
          ? "완전판 리포트 | 나의 사용설명서"
          : sp.locale === "id"
            ? "Laporan Lengkap | Panduan Kepribadian Saya"
          : "詳細レポート | ワタシのトリセツ",
    },
  };
}
...
const ID_SCORE_META: ScoreMeta[] = ID_RESULT_AXES.map((axis) => ({
  key: axis.dim,
  label: axis.title,
  low: axis.lowDescription,
  high: axis.highDescription,
}));
...
export default async function ReportPage({ params, searchParams }: PageProps) {
  ...
  if (previewType) {
    t32 = previewType;
    displayName = isKo ? "미리보기" : isId ? "Pratinjau" : "プレビュー";
    ...
    if (!paid) {
      return <LockedScreen token={token} locale={isKo ? "ko" : isId ? "id" : "ja"} />;
    }
  ...
  const idType = isId ? ID_RESULT_TYPES[t32] : null;
  const name = koType?.name ?? idType?.name ?? thirtyTwoName(t32);
  const essence = koType?.essence ?? idType?.essence ?? thirtyTwoEssence(t32);
  const catchphrase = koType?.oneLiner ?? idType?.oneLiner ?? thirtyTwoCatchphrase(t32);
  ...
  return (
    ...
    >
      <div className={styles.screenToolbar}>
        <p>{isKo ? "완전판 PDF 미리보기" : isId ? "Pratinjau PDF lengkap" : "自己分析PDF プレビュー"}</p>
        <ReportPrintButton locale={isKo ? "ko" : isId ? "id" : "ja"} />
      </div>

      <article className={styles.reportDocument}>
        <ReportCover
          locale={isKo ? "ko" : isId ? "id" : "ja"}
          profileLabel={isKo ? "PERSONALITY STORY" : isId ? "PROFIL KEPRIBADIAN" : "PERSONALITY PROFILE"}
          imageSrc={cutImagePath}
          imageAlt={name}
          characterName={name}
          title={essence}
          subtitle={isKo ? "나의 성격 스토리" : isId ? "Profil kepribadian" : "パーソナリティプロフィール"}
          metric={<CoverOcean scores={scores} locale={isKo ? "ko" : isId ? "id" : "ja"} />}
          quote={isKo ? `‘${catchphrase}’` : isId ? `“${catchphrase}”` : `「${catchphrase}」`}
          readerLabel={
            isKo
              ? greeting
                ? `${greeting}님의 리포트`
        ...
        <section className={styles.overviewPage}>
          <p className={styles.pageEyebrow}>{isId ? "PROFIL ANDA" : "YOUR PROFILE"}</p>
          ...
          <p className={styles.leadText}>
            {isKo ? (
              ...
              <>
                Laporan ini membaca lima kecenderungan dari penilaian diri bersama
                pola karakter <strong>{name}</strong>. Tidak ada kecenderungan yang
                mutlak baik atau buruk. Gunakan bagian yang terasa sesuai sebagai
                petunjuk untuk menemukan pilihan dan jarak hubungan yang nyaman bagi Anda.
              </>
            ) : (
              <>
          ...
          <div className={styles.profileStatement}>
            <p>{isKo ? `${essence} 유형의 핵심` : isId ? `Inti pola ${essence}` : `${essence}型の核`}</p>
            <strong>{catchphrase}</strong>
          </div>

          <div className={styles.scoreSection}>
            <div className={styles.sectionHeadingRow}>
              <h3>{isKo ? "5가지 성격 경향" : isId ? "Lima kecenderungan kepribadian" : "5つの傾向"}</h3>
              <span>
                {isKo ? "자기 진단 점수 / 10" : isId ? "Skor penilaian diri / 10" : "自己診断スコア / 10"}
              </span>
            </div>
            <div className={styles.scoreList}>
              {(isKo ? KO_SCORE_META : isId ? ID_SCORE_META : SCORE_META).map((meta) => (
                <ScoreRow
                  key={meta.key}
                  meta={meta}
          ...
          <nav className={styles.contents} aria-label={isKo ? "목차" : isId ? "Daftar isi" : "目次"}>
            <p className={styles.contentsLabel}>{isId ? "DAFTAR ISI" : "CONTENTS"}</p>
            <ol>
            ...
            >
              <header className={styles.chapterHeader}>
                <p className={styles.chapterNumber}>
                  {isId ? "BAB" : "CHAPTER"} {String(index + 1).padStart(2, "0")}
                </p>
                <h2>{chapter.title}</h2>
        ...
        <footer className={styles.endPage}>
          <p className={styles.pageEyebrow}>{isId ? "SIMPAN DEKAT-DEKAT" : "KEEP THIS CLOSE"}</p>
          <h2>
            {isKo ? "마음이 흔들릴 때마다, 다시." : isId ? "Kembali saat Anda membutuhkan arah." : "迷ったときに、何度でも。"}
          </h2>
          <p>
            {isKo
              ? "성격은 나를 단정 짓는 답이 아니라, 나를 이해하기 위한 지도예요. 상황과 경험에 따라 느끼는 방식이 달라졌을 때 이 리포트를 다시 열어 보세요."
              : isId
                ? "Kepribadian adalah peta untuk memahami diri, bukan vonis yang membatasi Anda. Buka kembali laporan ini ketika pengalaman baru mengubah cara Anda melihat diri sendiri."
                : "性格は、決めつけるための答えではなく、自分を理解するための地図です。状況や経験によって感じ方が変わったときは、またこのレポートを開いてください。"}
          </p>
          <div className={styles.endMark}>
            {isKo ? "나의 사용설명서" : isId ? "PANDUAN KEPRIBADIAN SAYA" : "ワタシのトリセツ"}
          </div>
          <Link
            href={`${isKo ? "/ko" : isId ? "/id" : ""}/me/${encodeURIComponent(previewType ? "preview" : token)}`}
            className={styles.backLink}
          >
            {isKo ? "완성된 자기 진단 결과 보기" : isId ? "Lihat hasil lengkap saya" : "埋まった自己診断結果を見る"}
          </Link>
        </footer>
          </>
        )}
      ...
      <div className="w-full max-w-md rounded-3xl border border-[#E8E1D5] bg-white px-8 py-10 text-center">
        ...
        <h1 className="mb-4 text-xl font-bold">
          {isKo ? (
            ...
            <>
              Laporan lengkap termasuk dalam
              <br />
              Edisi Lengkap
            </>
          ) : (
        ...
        <p className="mb-8 text-sm leading-relaxed text-[#6B6359]">
          {isKo
            ...
            : isId
              ? "Laporan panjang ini merangkum kekuatan, perhatian, hubungan romantis, pertemanan, dan karier tipe Anda. Setelah membeli Edisi Lengkap, Anda dapat mengunduhnya dari email dan halaman hasil."
              : "あなたのタイプの長所と短所、恋愛・友人関係、キャリアまでを一冊にまとめた長編レポート。全解放プランを購入すると、メールでお届けします。"}
        </p>
        <Link
          href={`${isKo ? "/ko" : isId ? "/id" : ""}/me/${encodeURIComponent(token)}`}
          className="inline-block rounded-full bg-[#2A2520] px-8 py-3 text-sm font-semibold text-[#FAF7F2]"
        >
          {isKo ? "내 결과에서 자세히 보기" : isId ? "Lihat di hasil saya" : "マイ図鑑で詳しく見る"}
        </Link>
      </div>
    </main>
  );
