...
export default function FriendDiagnosisPage({
  params,
  locale = "ja",
}: {
    ...
    <Suspense
      fallback={
        <div className="min-h-screen bg-[#E4E0F5] flex flex-col flex-1 items-center justify-center">
          <p className="text-sm font-bold text-[#2E2E5C]/70">{copy.loading}</p>
        </div>
      }
    >
      <FriendContent inviteCode={inviteCode} locale={locale} />
    </Suspense>
  );
}
...
function FriendContent({
  ...
  const [perceiverName, setPerceiverName] = useState<string>(copy.friend);
  // ③ 本人へのメッセージ (任意・最大200字)
  ...
  const subjectLabel = owner.displayName?.trim() || copy.friend;
  ...
  return (
    <ErrorScreen
      message={submitError ?? copy.submitErrorTitle}
      onRetry={submit}
      locale={locale}
    />
  );
      ...
      <div className="flex flex-col flex-1 min-h-screen pb-12 bg-white">
        ...
        <main className="flex flex-col flex-1 w-full pt-6 pb-4">
          ...
          {questions.map((q) => {
            ...
            return (
              <div
                ...
                tabIndex={-1}
                aria-label={copy.questionAria(q.id)}
              ...
              >
                <p className="mx-auto mb-7 max-w-3xl text-center text-[17px] font-bold leading-relaxed text-[#2E2E5C] sm:text-[20px]">
                  {friendQuestionText(locale, q, inviteeName)}
                </p>
                <LikertScale
                  value={scaleAnswers[q.id]}
                  onChange={(v) => answerAndAdvance(q.id, v)}
                  leftLabel={copy.scaleLeft}
                  rightLabel={copy.scaleRight}
                  optionLabels={copy.scaleOptions}
                  size="lg"
                />
              </div>
            );
          ...
          <div className="mx-auto mt-10 flex w-full max-w-[1080px] flex-col items-center gap-3 px-4 md:px-8">
            ...
            >
              {isLastPage ? copy.finishQuestions : copy.next}
            </button>
            {page > 0 && (
