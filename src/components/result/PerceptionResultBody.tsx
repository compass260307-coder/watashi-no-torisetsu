...
const CARD = "bg-white rounded-3xl border-2 border-[#0094D8]/25 shadow-md p-6";
  ...
  return (
    <>
      ...
      <section className="mb-8">
        <SectionHead num={1} title={`${p}さんから見た${youWord}`} />
        <div className={CARD}>
          <div className="text-center">
            <p className="text-[#5B5BEF] font-bold text-sm mb-1">相互理解度</p>
            <p className="text-[#2E2E5C] font-black text-6xl leading-none">
              {view.mutual}
            ...
            {isIndividual && rankNote ? (
              <p className="text-[#5B5BEF] text-sm font-black mt-2 leading-relaxed">
                {rankNote}
              </p>
            ) : (
          ...
          <p className={PERCEPTION_BODY_TEXT_CLASS}>{view.perceivedLookBody}</p>
        </div>
      </section>

      {/* ===== ② ギャップ (レーダー + 共通発散バー + 軸ごと解説文) ===== */}
      <section className="mb-8">
        <SectionHead num={2} title={`${p}さんとのギャップ`} />
        <div className={CARD}>
      ...
      {isIndividual ? (
        ...
        (trimmedOwnerMessage.length > 0 || view.qualEntries.length > 0) && (
          <section className="mb-8">
            <SectionHead num={3} title={`${p}さんからの贈りもの`} />
            <div className="flex flex-col gap-4">
              {trimmedOwnerMessage.length > 0 && (
                <figure className="rounded-3xl bg-[#F4F4FE] px-6 py-6">
                  <blockquote className="text-[#2E2E5C] font-black text-xl leading-[1.5] whitespace-pre-wrap break-words">
                    {trimmedOwnerMessage}
                  </blockquote>
                  <figcaption className="text-[#2E2E5C]/60 text-sm font-bold mt-3 text-right">
                    — {p} より
                  </figcaption>
                </figure>
              )}
              {view.qualEntries.map((e) => (
                <div key={e.label} className={CARD}>
                  <p className="text-[#5B5BEF] font-bold text-xs mb-1">
                    {e.label}
                  </p>
                  <p className="text-[#2E2E5C] font-black text-lg leading-[1.5] whitespace-pre-wrap break-words">
                    {e.value}
                  </p>
                </div>
              ))}
      ...
      {!isIndividual && view.hasFound && (
        <section className="mb-8">
          <SectionHead num={3} title={`${p}さんが見つけたアナタ`} />
