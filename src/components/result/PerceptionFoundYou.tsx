...
//
// ★ Day 12 ③④改修で /evaluate/result からの参照を撤去 (バッジ/吹き出し UI が
//   「色付き箇条書き」に見えるため、③ は文章カード PerceptionFoundProse に全面改装)。
//   温存方針 (Stripe コード同様、後日の有料深掘りレポートで再利用) に従い削除しない。
//
    ...
    <div className="bg-white rounded-3xl border-2 border-[#0094D8]/25 shadow-md p-6">
      ...
      <ul className="flex flex-col gap-2">
        {strengths.map((s) => (
          <li
            key={s.title}
            className="flex items-center gap-3 rounded-2xl border-2 border-[#5B5BEF] bg-[#5B5BEF]/25 p-3"
          >
            <span
              aria-hidden="true"
              className="flex-shrink-0 w-10 h-10 rounded-full bg-[#5B5BEF] flex items-center justify-center"
            >
              <MedalIcon className="w-6 h-6 text-[#2E2E5C]" />
            </span>
            <div>
              <p className="text-[#2E2E5C] font-black text-base leading-snug">
                {s.title}
              </p>
              <p className="text-[#2E2E5C]/75 text-xs leading-relaxed mt-0.5">
                {s.body}
              </p>
            </div>
          </li>
        ...
        {surprises.map((s) => (
          <li key={s.title} className="flex items-start gap-2">
            ...
            <div className="flex-1 bg-[#EFF7FD] rounded-2xl rounded-tl-md px-4 py-3">
              <p className="text-[#2E2E5C] font-black text-xs mb-0.5">
                {s.title}
              </p>
              <p className="text-[#2E2E5C]/85 text-xs leading-relaxed">
                {s.body}
              </p>
            </div>
          </li>
