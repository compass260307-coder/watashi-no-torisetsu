...
const OPEN_BODY: Record<BigFiveDimension, { high: string; low: string }> = {
  O: {
    high: "新しいもの好きは、ふたり公認。「これ面白いよ」と最初に見つけてくるのはいつもあなたで、{v}もそれを楽しみにしてる。話題の引き出しとして、静かに頼られてるよ。",
  ...
  C: {
    ...
    low: "きっちりより、マイペース。それはふたり公認の持ち味だよ。あなたの力の抜け具合があるから、一緒にいる時間の空気がゆるんで、{v}も肩の力を抜けてるんだ。",
  },
  E: {
    high: "場を明るくする力は、自他ともに認める持ち味。あなたがいる日といない日で集まりの空気が変わること、{v}はちゃんと知ってるよ。",
  ...
  N: {
    ...
    low: "ちょっとやそっとじゃ動じないどっしり感は、自分から見ても{v}から見てもブレてない。まわりが慌ててるときほど、あなたの落ち着きが効いてるよ。",
  },
};

// 盲点の窓 (強): 友達の方が15pt以上高く見てる軸の本文。
const BLIND_BODY: Record<BigFiveDimension, string> = {
  O: "自分では「普通のこと」のつもりでも、{v}の目にはあなたの好奇心がキラキラ映ってるみたい。新しい話題や面白いものを見つけてくる姿が、一緒にいる時間を飽きさせないんだ。本人だけが気づいてない魅力だよ。",
  C: "「この人、ほんとにちゃんとしてる」って、{v}の中でのあなたは自分の想像以上。自分では当たり前にやってることが、外からは努力と誠実さに見えてる。そのギャップの分だけ、株が上がってるよ。",
  E: "自分では控えめにしてるつもり。でも{v}には、あなたは場を明るくする人に見えてるよ。何気ないひと言や笑い方が、思ってる以上に空気を変えてる。無自覚なのがまたいいんだ。",
...
const SOFT_BLIND_BODY: Record<BigFiveDimension, { high: string; low: string }> =
  {
    O: {
      high: "{v}の回答では、あなたの好奇心がかなりハッキリ出てた。「次は何を見つけてくるんだろう」って、ちょっと楽しみにされてるみたい。その期待、悪くないでしょ。",
      low: "{v}の回答からは、あなたの「じっくり味わう派」なところがくっきり見えてた。流行に流されない選び方が、{v}の中ではあなたらしさになってるよ。",
    },
    C: {
      high: "{v}の回答では、あなたの「ちゃんとしてる度」がかなり高めに出てた。細かいところまで見られてないようで、見られてる。信頼されてる証拠だよ。",
      low: "{v}の回答からは、あなたのマイペースさがのびのび伝わってた。窮屈じゃない空気をつくれる人として映ってるみたい。それ、なかなか出せない味だよ。",
    },
    E: {
      high: "{v}の回答では、あなたの明るさがかなり前に出てた。本人が思う「素の自分」より、外に届いてるエネルギーは大きいのかも。",
      low: "{v}の回答からは、あなたの聞き役っぷりがくっきり見えてた。騒がしくないのに存在感がある。そういう人、実は少ないんだよ。",
    },
    A: {
      high: "{v}の回答では、あなたのやさしさがかなり強めに出てた。隠してるつもりでも、行動の端々からこぼれてるみたい。",
      low: "{v}の回答からは、あなたの「自分を持ってる感」がハッキリ出てた。合わせすぎない距離感が、逆に信頼につながってるよ。",
    },
    N: {
      high: "{v}の回答では、あなたの繊細なアンテナがしっかり拾われてた。細かい変化に気づく人だって、ちゃんと知られてるよ。",
      low: "{v}の回答からは、あなたのどっしり感がくっきり出てた。「この人は慌てない」って安心されてるみたい。頼られる才能だよ。",
    },
  };

// 秘密の窓 (強): 自分の方が15pt以上高く感じてる軸の本文。
const SECRET_BODY: Record<BigFiveDimension, string> = {
  O: "頭の中でぐるぐる広がってる好奇心や妄想、まだ{v}には見せてないみたい。あなたの脳内には、外から見えてる何倍もの世界が広がってる。ふとした瞬間にそれを話したら、{v}はきっと驚くよ。",
...
> = {
  ...
  C: {
    ...
    low: "「ゆるくやりたい」という本音、自分の中でははっきりしてる。{v}の前でどこまで出すかは、あなたが決めていいんだよ。",
  },
  E: {
...
const UNKNOWN_ITEMS: Record<BigFiveDimension, { title: string; body: string }> =
  {
    O: {
      title: "新世界にどハマりする自分",
      body: "まだ出会ってないジャンルに、ある日突然どハマりする可能性。ふたりとも知らない熱中モードのあなたが、この窓の奥で出番を待ってるよ。",
    },
    C: {
      title: "本気の仕切り屋モード",
      body: "イベントの幹事や旅行の計画、任されたら意外とハマるかも。段取りで頼られるあなたは、まだどっちの目にも映ってない。",
    },
    E: {
      title: "フルスロットルの自分",
    ...
    A: {
      title: "甘え上手な自分",
      body: "いつも気づかう側のあなたが、思いきり頼って甘える姿。それを見せる日が来たら、{v}との関係はもう一段深くなるよ。",
    },
    N: {
      title: "感情を言葉にする自分",
