...
export function UnlockCard({
  ...
  return (
    <div className="bg-[#FFF9F0] border-[3px] border-[#3A2D6B] rounded-[24px] p-5 mb-8">
      ...
      {/* 解放項目の箇条書き 3 点 (check SVG + 太字リード) */}
      <ul className="flex flex-col gap-2.5 mb-5">
        {bullets.map((b) => (
          <li key={b} className="flex gap-2 items-start">
            <CheckIcon className="w-4 h-4 text-[#FE3C72] flex-shrink-0 mt-0.5" />
            <span className="text-[#3A2D6B] font-bold text-sm leading-snug">
              {b}
            </span>
          </li>
        ))}
