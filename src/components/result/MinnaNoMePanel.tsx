...
export function MinnaNoMePanel(props: MinnaNoMePanelProps) {
  ...
  return (
    <div className="px-1 pt-1 pb-2">
      ...
      {/* ギャップ (数値なし・言葉のみ) */}
      {props.gapSentence && (
        <p className="body-gothic text-[#1A1A1A] font-normal text-[17px] leading-[1.4] mb-6">
          {props.gapSentence}
        </p>
      )}
      ...
      {(props.letters.length > 0 || props.favoritePoints.length > 0) && (
        <section>
          ...
          {props.letters.map((l, i) => (
            ...
            >
              <blockquote className="body-gothic text-[#1A1A1A] font-normal text-[16px] leading-[1.4]">
                {l.message}
              </blockquote>
