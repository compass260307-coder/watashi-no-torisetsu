// プレミアム化 v2 Week 1 T1-7 パターン 3 疎通テスト
// 「自己評価 + 友達 3 人 (異なる角度)」での AI 統合トリセツ生成を最小チェーンで叩く。
//
// 友達の角度:
//   - ユウキ (繊細系): N が低く見えている = 「落ち着いて見える」
//   - ヒロ   (実家系): E が低く見えている = 「もっと内向に見える」
//   - アヤ   (自由系): O がさらに高く見えている = 「もっと冒険的に見える」
//
// 実行: npx tsx scripts/smoke-t1-7-pattern3.ts
...
async function main() {
  ...
  console.log("=== 疎通テスト T1-7 パターン 3: 自己 + 友達 3 人 ===");
  ...
  console.log(
    "  " +
      (out.chapters.multifacetedness?.body ?? "")
        .replace(/\s+/g, " ")
        .slice(0, 200) +
      "...",
  );
  ...
  console.log("\n--- 第五章 (対人関係パターン) 冒頭 200 字 ---");
  console.log(
    "  " +
      (out.chapters.relationships?.body ?? "")
        .replace(/\s+/g, " ")
        .slice(0, 200) +
      "...",
  );
